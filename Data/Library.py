...
class Library:
    ...
    def ngramify(self, indices, n=2):
        seq_length = indices.shape[0]
        if seq_length < n:
           raise ValueError("Sequence length must be greater than or equal to n.")
        new_indices = torch.zeros([seq_length - n + 1, n], dtype=torch.long)
        for nx in range(n):
            new_indices[:, nx] = indices[nx:seq_length - n + nx + 1]
        return new_indices
