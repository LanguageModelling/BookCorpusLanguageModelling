...
class BookCorpusDataset(Dataset):
    def __init__(self, file_path = None, chunk_size=1000, random_sample=False):
    ...
    def _read_stream_in_chunks(self, iterable_dataset):
        for batch in iterable_dataset.batch(batch_size=self.chunk_size):
            yield pd.DataFrame.fromrecords(batch.to_pandas().to_dict(orient='records'))
    ...
    def __getitem__(self, idx):
        ...
        if self.sample_count >= self.total_examples:
...
if __name__ == "__main__":
    ...
    dataset = BookCorpusDataset("bookcorpus.parquet", chunk_size=1000)
